...
export const ProcessStep = ({ stepNumber, title, description, isFirst, isLast, delay = 0 }: ProcessStepProps) => {
  return (
    <ScrollReveal delay={delay} className={`flex flex-col md:flex-row items-start md:items-baseline py-8 md:py-10 border-neutral-200 border-b ${
        isFirst ? "border-t" : ""
      } ${isLast ? "border-b-0" : ""}`}
    >
      <div className="text-black/40 font-['Instrument_Sans'] text-sm tracking-widest mb-4 md:mb-0 md:mr-8 md:w-8 shrink-0">
        {stepNumber}
      </div>
